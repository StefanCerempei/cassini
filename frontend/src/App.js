...
function AppContent() {
    ...
    const generateMockSpills = () => [
        ...
        { id: 2, lat: 48.4680, lon: 27.5850, severity: 'medium', score: 0.68, area_m2: 580,  type: 'petrol', timestamp: new Date(Date.now() - 2*3600*1000).toISOString(), description: 'Pată petrol în aval' },
        { id: 3, lat: 48.4750, lon: 27.5790, severity: 'low',    score: 0.42, area_m2: 230,  type: 'petrol', timestamp: new Date(Date.now() - 5*3600*1000).toISOString(), description: 'Urmă de petrol la suprafață' },
        { id: 4, lat: 48.4650, lon: 27.5880, severity: 'medium', score: 0.71, area_m2: 340,  type: 'petrol', timestamp: new Date(Date.now() - 8*3600*1000).toISOString(), description: 'Petrol în derivă' },
    ];
    ...
    const navItems = [
        ...
        { id: 'analysis',  icon: '🧪', label: 'Analiză NDWI',    desc: 'Detecție poluare comparativă' },
                                ...
                                <div key={spill.id} className={`spill-card ${spill.severity}`}>
                                    <div className="spill-header">
                                        <div>
                                            ...
                                            <div className="spill-badge">{spill.severity === 'high' ? 'URGENȚĂ' : spill.severity === 'medium' ? 'ATENȚIE' : 'MONITORIZARE'}</div>
                                        </div>
                    ...
                    <div className="page analytics-page">
                        <div className="analytics-grid">
                                ...
                                <div className="chart-container">
                                    {historicalData.map((d, i) => (
                                        <div key={i} className="chart-bar">
                                            ...
                                            <div className="bar-container"><div className="bar" style={{ height: `${d.spills*30}px` }}></div></div>
                            ...
                            <div className="analytics-card">
                                ...
                                <div className="pie-legend"><span>🔴 {stats.high}</span><span>🟠 {stats.medium}</span><span>🟡 {stats.low}</span></div>
                            </div>
                            <div className="analytics-card">
                            ...
                            {spills.filter(s => s.severity === 'high').map(alert => (
                                <div key={alert.id} className="alert-card critical">
                                    ...
                                    <div className="alert-content">
                                        ...
                                        <div className="alert-details"><span>📍 {alert.lat}, {alert.lon}</span><span>📐 {alert.area_m2} m²</span></div>
                                    </div>
